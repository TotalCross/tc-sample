...
public class AccordionSample extends ScrollContainer {
	...
	public void initUI() {
		...
		for(int i = 1; i < ac.length; i++) {
			add(ac[i], CENTER, AFTER + UnitsConverter.toPixels(Control.DP + 2), SCREENSIZE + 85, PREFERRED);
		...
		add(warning, LEFT + gap, AFTER + gap/2, FILL - gap, PREFERRED);
		
		Button confirm = new Button("SUBMIT");
		confirm.setBackForeColors(Colors.P_600, Colors.ON_P_600);
		confirm.setEnabled(false);
		add(confirm, CENTER, AFTER + gap/2, confirm.getPreferredWidth() <= 32 ? DP + 64 : confirm.getPreferredWidth() + (int) Settings.screenDensity*100*32/100, DP + 36);
