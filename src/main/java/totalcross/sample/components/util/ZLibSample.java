...
public class ZLibSample extends BaseScreen {
	private Button btn;
	...
	public void onContent(ScrollContainer content) {
		...
		content.add(btn = new Button("    Start    "), CENTER, TOP + 3);
	...
	private void proceed(int compression_level) {
		...
		content.add(new Label("original size is " + sb.length()), LEFT + gap, AFTER + gap);
		...
		try {
			...
			content.add(new Label("compressed output size=" + sz), LEFT + gap, AFTER + gap);
			content.add(new Label("Elapsed: " + (end - ini) + "ms"), LEFT + gap, AFTER + gap);

		} catch (IOException e) {
			content.add(new Label("an exception occurred : " + e.getMessage(), LEFT, Colors.ERROR, true), LEFT + gap, AFTER + gap);
		}
		try {
			cs.setPos(0);
		} catch (IOException exc) {
			content.add(new Label("an exception occurred : " + exc.getMessage(), LEFT, Colors.ERROR, true), LEFT + gap, AFTER + gap);
		}
		content.add(new Label("compressed byte stream:"), LEFT + gap, AFTER + gap);
		...
		while (left > 0) {
			...
			content.add(new Label(ofs + " - " + sb.toString()), LEFT + gap, AFTER + gap);
		...
		try {
			...
			content.add(new Label("uncompressed output size=" + sz), LEFT + gap, AFTER + gap);
			content.add(new Label("Elapsed: " + (end - ini) + "ms"), LEFT + gap, AFTER + gap);
		...
		if (sz > 0) {
			...
			content.add(new Label("original content was '" + str.substring(0, 20) + "'"), LEFT + gap, AFTER + gap);
			content.add(new Label("original size was " + sz), LEFT + gap, AFTER + gap);
		}
	}
}
